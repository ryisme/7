...
class Aimbot:
    ...
    def __init__(self, box_constant=400, collect_data=False, mouse_delay=0.0000, debug=False):
        ...
        print(colored('''[INFO] LOADING...''',"yellow"))
        self.model = torch.hub.load('ultralytics/yolov3', 'custom', path='lib/test.pt', force_reload=True)

        if torch.cuda.is_available():
            print(colored("GPU [ENABLED]", "red"))
        else:
    ...
    def start(self):
        print(colored('''[INFO] Succesfully Loaded!''',"yellow"))
            ...
            if len(results.xyxy[0]) != 0:  # player detected
                ...
                for *box, conf, cls in results.xyxy[0]:  # iterate over each player detected
                    ...
                    if crosshair_dist <= least_crosshair_dist and not own_player:
                        ...
                        closest_detection = {"x1y1": x1y1, "x2y2": x2y2, "relative_head_X": relative_head_X,
                                             "relative_head_Y": relative_head_Y, "conf": conf}

                    if not own_player:
                        cv2.rectangle(frame, x1y1, x2y2, (244, 113, 115),
                                      1)  # draw the bounding boxes for all of the player detections (except own)
                ...
                if closest_detection:  # if valid detection exists
                    cv2.circle(frame, (closest_detection["relative_head_X"], closest_detection["relative_head_Y"]), 5,
                               (115, 244, 113), -1)  # draw circle on the head

                    absolute_head_X, absolute_head_Y = closest_detection["relative_head_X"] + detection_box['left'], \
